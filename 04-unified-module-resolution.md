...
that the spec does not define, duplicates its logic across three code paths, and
treats an exact whole-name match as ambiguous whenever the name is also a
substring of a longer name. Refactor resolution so a single, name-only engine
...
2. A short-name tier matches a trailing path segment as if exact:
   ...
   fallback.
3. No three-character floor. The spec's `start task rv` must error before the
   fallback search; today it does not.
...
3. Run the exact-whole-name tier first for every non-path input, bare or
   ...
   character floor. On the cross-category surfaces, an exact match in two
   categories is two matches and is ambiguous; it falls to selection and may be
   resolved by category-qualifying the name.
...
5. Collect matches from installed config and the registry index as two sources
   ...
   resolution proceeds against installed config alone, and a registry-only
   identifier that cannot be fetched falls through to a not-found error. An exact
   installed match on a category-specific surface may short-circuit before the
   registry fetch, since names are unique within a category.
...
6. Reduce the fallback match set to one decision: zero matches is a not-found
   ...
   entry as `category:name` with its source (installed or registry). The listed
   `category:name` forms are valid arguments that round-trip back to the same
   entry. The menu accepts an entry number or a typed name that uniquely
   identifies one shown entry. Use one selection implementation for all surfaces.
...
8. Decouple resolution from the search scoring engine. Resolution must not call
   `SearchInstalledConfig`, `SearchCategoryEntries`, `matchScorePatterns`,
   `ParseSearchTerms`, or `CompileSearchTerms`. Those remain in use by
   `start search` and must keep their current behaviour. Resolution gets its own
   name-only matching over installed config fields and registry index entries.
...
- Resolution matching is literal and case-insensitive over names only: equality
  for exact, `strings.Contains` for substring, `strings.HasPrefix` for prefix.
  No regular expressions, no description or tag matching, no multi-term splitting.
...
- Error classification and exit codes stay consistent with the current
  convention: a genuine absence is a not-found error (`notFoundError`); an
  ambiguity without a TTY and a too-short query are usage errors (`usageError`).
  See `internal/cli/exitcodes.go` and `start help schemas`.
...
2. Build the unified resolver flow on top of the primitive. Parameterise it by
   ...
   An exact installed match on a single category short-circuits before the
   registry fetch.
...
3. Collapse selection into one implementation. Replace `selectSingleMatch`/
   ...
   `promptTaskSelection` with a single selector that renders entries as
   `category:name` with source, handles the non-TTY error list, and accepts a
   number or a uniquely-identifying typed name.
...
4. Route `--role` and `--agent` through the unified resolver (single category;
   ...
   `resolveCrossCategory`'s bespoke tier logic.

5. Route `start task` through the unified resolver. Keep the file-path read, the
   template processing, and the task-role follow-up (`GetTaskRole` and its own
   resolution), but replace the inline installed/registry search and selection
   with the shared engine. Remove `findInstalledTasks`, `findRegistryTasks`,
...
6. Reimplement `resolveContexts` as a loop over terms that, per term, handles the
   path bypass and the `none`/`default` sentinels, then delegates to the unified
   single-match resolver. Remove `contextScoreThreshold`, the multi-select
...
7. Remove now-dead resolution helpers: `findExactInstalledName`,
   ...
   remains. Confirm `mergeModuleMatches` either moves into the new engine with
   `category:name` de-duplication or is replaced.
...
- A category prefix does two things at once: it scopes the category and switches
  the fallback to prefix. On a category-specific surface, adding the surface's
  own prefix is therefore not a no-op — `start task review` (substring) and
  `start task tasks:review` (prefix) can return different sets. Preserve that.
...
2. `start task rv` errors with a too-short-query message and never reaches the
   ...
   resolves via the exact tier despite the two-character name.
...
5. A registry-only module whose exact name is typed is installed and then used,
   ...
   same identifier falls through to a not-found error and installed modules still
   resolve.
6. On `get`/`describe`, a bare name that is an exact match in two categories
   produces a selection menu on a TTY and a non-TTY error listing both as
   `category:name`; category-qualifying the name resolves it without a menu.
7. Each `--context` term resolves to a single module via the match rule; a term
   that matches several menus or errors, and an explicit term that matches
   nothing is a not-found error. The score threshold no longer affects results.
8. Resolution contains no call to `SearchInstalledConfig`,
   `SearchCategoryEntries`, `matchScorePatterns`, `ParseSearchTerms`, or
   `CompileSearchTerms`. `start search` output is unchanged.
9. The task, role, agent, context, and cross-category surfaces share one
...
10. `scripts/invoke-tests` passes and the `integration`, `e2e`, and `registry`
