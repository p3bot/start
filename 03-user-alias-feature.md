...
- A global-only alias store (`aliases/aliases.cue`, a managed file in an `aliases/` subdirectory of the global config dir) and a trivial embedded structural schema for it.
...
VERIFIED, DO NOT RE-TEST (cuelang.org/go v0.16.1): the directory loader's `load.Instances([]string{"."}, cfg)` call (`internal/cue/loader.go`, `loadDir`) is non-recursive — it builds only the `.cue` files directly in the scope directory and never descends into subdirectories. A managed store at `aliases/aliases.cue` is therefore excluded from every directory package build automatically, with no change to the shared directory loader. A malformed `aliases/aliases.cue` beside valid config files does not fail the build, and the subdirectory's fields never appear in the merged value. This is pinned by `TestLoader_SubdirectoryNotRecursed` in `internal/cue/loader_test.go`, which covers a malformed subdir file not breaking a sibling load, a valid subdir file not leaking its fields, and a dir whose only `.cue` content is in the subdir loading as empty. A global directory whose only `.cue` content lives under `aliases/` has no top-level `.cue` files, so `HasCUEFiles` returns false and the loader skips the directory as empty. This is the load-bearing resilience guarantee; it is settled and needs no further investigation or test.
...
`internal/cli/config_export.go` holds `printCueFiles`, which lists the scope directory with a non-recursive `os.ReadDir` (skipping `entry.IsDir()`) and streams each top-level `.cue` file's raw bytes. Being non-recursive and files-only, it never descends into the `aliases/` subdirectory.
...
1. Alias store. Aliases are stored only in global config, in a managed file `aliases/aliases.cue` (the file `aliases.cue` inside an `aliases/` subdirectory of the global config dir) holding a single top-level `aliases:` field that maps an alias name to a list of token strings. The subdirectory excludes the file from every package build automatically, because the directory loader is non-recursive (see Current State, VERIFIED). The store is never read through the directory loader; every alias-consuming surface compiles the single file in isolation via `ctx.CompileBytes` and looks up the `aliases` field. The user-facing category is `aliases` and the command is `alias`. The store lives under the global config directory. Any operation that may create the store — `set`, `import`, and `open` — first ensures the `aliases/` subdirectory exists via `os.MkdirAll`. `set` and `import` write through the CUE API, so they create the subdirectory as part of the write; `open` hands the path to an external `$EDITOR` rather than writing through our code, so it must create the subdirectory itself beforehand (and seed a minimal `aliases: {}` when the store is absent) — otherwise a first-run save fails because the parent directory does not exist. Read and write go through the CUE Go API; write formats with CUE's formatter so output is canonical and token strings (including embedded quotes, commas, and colons) round-trip losslessly. Do not hand-roll CUE serialisation or copy `writeSettingsFile`'s string-builder approach.
...
3. Set command. `start alias set <name> <token>...` uses `DisableFlagParsing`, so everything after `<name>` is captured verbatim as the value; `set` has no own flags, and global flags do not apply to it. Help is available via `start help alias set` and via `start alias set --help` (a leading `-h`/`--help` is caught by the extended `checkHelpArg` — see Current State — before the args are treated as data). Set is an upsert: it creates or replaces one alias and preserves all others. All rejections write nothing and print a specific hint with a valid example:
   ...
   Before overwriting `aliases/aliases.cue`, guard against clobbering content the tool cannot safely round-trip: if the existing file fails to parse, or parses but contains non-`aliases` top-level keys, refuse to write and direct the user to fix or edit it manually. Fail closed.
...
6. Management command. A top-level command `alias` (with cobra alias `aliases`), global config only:
   ...
   start alias open                  open aliases/aliases.cue in $EDITOR
   start alias export                print aliases/aliases.cue to stdout
...
10. Subdirectory isolation (no filtering needed). Because the store lives at `aliases/aliases.cue`, no enumeration surface needs an exclusion filter. Every surface that enumerates `.cue` files directly — `printCueFiles` (`config export`), `checkConfigDir`'s listing in `internal/doctor/checks.go`, `config.CUEFilesInDir` in `internal/config/validation.go` (feeding doctor's schema validation), and the `configDir/*.cue` globs in `autosetup.go` and `install.go` (feeding `IdentifyBrokenFiles`) — uses a non-recursive `os.ReadDir` with a files-only `entry.IsDir()` skip, or a non-recursive `*.cue` glob, so a subdirectory is never visited. CUE's own package loader is likewise non-recursive (Current State, VERIFIED). The store therefore stays out of generic config tooling automatically; do not add any `_`-prefix or path filter to these surfaces. The dedicated alias-store section (Requirement 9) remains the single authority on alias-store health; no generic config or doctor surface reports on `aliases/aliases.cue`. Beyond the alias command itself, no config command is extended for aliases.
...
11. Tests. Cover:
    ...
    - Import: merge upsert (export→import round-trip is a no-op), `--replace` overwriting the store, stdin and file sources, and atomic fail-closed rejection on an invalid entry, a parse failure, or non-`aliases` keys. Both modes also refuse — writing nothing — when the existing `aliases/aliases.cue` is unparseable or carries non-`aliases` top-level keys, including the `--replace`-against-a-non-`aliases`-existing-store case.
    - Subdirectory isolation on every direct `.cue` enumeration: with a malformed `aliases/aliases.cue` present, `config export` does not emit it; doctor's Configuration listing does not show it; doctor's schema-validation sweep does not compile it; and the `autosetup`/`install` broken-file diagnostics do not list it even when other config is broken — because each surface is non-recursive, not because of any filter. Confirm a malformed store leaves the main directory load and every other subcommand unaffected. The `doctor` alias check (count, absent store, malformed store).
...
- Read the store by compiling the single `aliases/aliases.cue` file in isolation (`ctx.CompileBytes(data, cue.Filename(path))`) and looking up the `aliases` field; never load it through the directory loader. Write by constructing a CUE value or syntax tree and formatting with CUE's formatter. Do not hand-roll CUE serialisation.
- The managed `aliases/aliases.cue` holds only the `aliases` field. Before overwriting, refuse to write if the existing file fails to parse or contains non-`aliases` top-level keys, directing the user to fix or edit it manually. Fail closed; never silently discard unrecognised content.
...
- A value token containing commas, colons, spaces, and embedded quotes is stored verbatim, round-trips through `aliases/aliases.cue`, and reaches the target argument unchanged on resolution; token case is preserved while the alias name is lowercased.
...
- `aliases/aliases.cue` is written and read via the CUE API and contains only the `aliases` field; a file containing other top-level keys is not overwritten.
...
- A malformed `aliases/aliases.cue` does not break `start task ...`, bare `start`, or any other subcommand, and a global directory whose only `.cue` content is `aliases/aliases.cue` loads as empty. A present-but-invalid store surfaces a resolver error only when the typed first token matches the malformed alias's name; an unrelated unknown token still yields `unknown command`.
- `start config export` does not include `aliases/aliases.cue` in its output.
- `start doctor` does not list `aliases/aliases.cue` in its Configuration section (even when the store is malformed) and does not compile it on the schema-validation path; the alias-store section is the only place the store's health is reported.
- `start alias open` opens the global `aliases/aliases.cue`; `start alias export` prints it to stdout.
