...
- Composition: one alias may set either a task or a fixed prompt, plus role, agent, and contexts in a single invocation.
- A `prompt` value type: an alias whose subcommand slot is a fixed, free-text prompt resolving to `start prompt <text>`.
...
- Unqualified alias values. Every target carries an explicit category prefix; the four module categories take a `category:path` address and `prompt:` takes free text. A colon-less value is rejected.
- In-command expansion. `start task pc` does not consult aliases; only the top-level `start pc` form does.
- Arbitrary command strings or shell fragments as values. The only free-text target is `prompt:` content, passed verbatim as the `prompt` subcommand's single argument; every other target is a typed module address, and no value is ever interpreted or re-parsed as a shell command.
...
- `01-prompt-arg-feature.md` — prerequisite. Prompt-valued aliases rely on the multi-argument `prompt` command from that project so a trailing positional concatenates into the fixed prompt with no alias-layer special-casing. Land 01 before the prompt-alias paths here.
...
2. Value format. A value is composed of one or more targets, each carrying an explicit category prefix. There are two kinds of target:

   - Module address: `category:path` where category is one of `agents`, `roles`, `contexts`, `tasks`. `path` matches the module-name shape: lowercase, kebab-case segments, slash-separated.
   - Prompt: `prompt:text` where `text` is free-text prompt content passed verbatim to the `prompt` subcommand.

   Tokenisation splits on the known category anchors (`agents:`, `roles:`, `contexts:`, `tasks:`, `prompt:`), not on a comma separator. This is unambiguous because a module path never contains a colon (`parseAddress` splits on the first colon and the path is colon-free), so every `knowncat:` occurrence in the structured part begins a new target. A comma or whitespace between targets is tolerated as a cosmetic separator and trimmed; it is not load-bearing. An unknown `foo:` is not an anchor, so it is absorbed into the preceding target's value and fails address validation, rejecting the `set`.

   `prompt:` is terminal: once the tokeniser sees it, the remainder of the value — including any commas, colons, or category-looking substrings — is the prompt text, taken verbatim. This is the one target whose value may legitimately contain those characters, so it must appear last (or simply be defined as consuming to end of value). Quotes inside the prompt are ordinary shell escaping at input time and pass through unchanged: execution rewrites argv and injects the stored prompt as a single positional token with no second shell parse, so embedded quotes/commas/spaces reach the `prompt` argument verbatim.

   Every target must carry an explicit category prefix; a colon-less value is rejected on `set` (note `parseAddress` accepts a bare name with no error, so the qualification check is separate from its category validation). Alias names and module-address values are case-insensitive and normalised to lowercase on write; an uppercase input resolves identically to its lowercase form. Prompt text is the exception: it is stored verbatim with case preserved, since it is content rather than an identifier. Path existence is not validated — a well-formed alias whose target does not exist resolves exactly as if the user had typed that command, driving the normal search/selection/not-found behaviour.

3. Composition and cardinality. A value may compose multiple targets. Cardinality limits are enforced in Go, because the embedded `#Aliases` schema covers address shape only and CUE cannot express per-category list cardinality cleanly (see Requirement 8): at most one `tasks:`, at most one `prompt:`, at most one `roles:`, at most one `agents:`, and any number of `contexts:`. Additionally, `tasks:` and `prompt:` are mutually exclusive — both occupy the subcommand slot (Requirement 4), so a value carrying both is rejected. A value breaching any limit is rejected on `set`. The same Go check runs on the resolve path and in `doctor`, so a hand-edited store that breaches a limit is caught — reported by `doctor`, surfaced as an entry error by the resolver per Requirement 6 — rather than silently expanded.
...
4. Dispatch. An alias value maps to a single `start` invocation:
   ...
   | `prompt:T` | `prompt T` (the subcommand, with `T` as its single positional argument) |
   | `roles:X` | `--role X` |
   | `agents:X` | `--agent X` |
   | `contexts:X` | `--context X` (one per context) |

   `tasks:` and `prompt:` share the subcommand slot and are mutually exclusive (Requirement 3). Token order is deterministic: the `task` or `prompt` subcommand first when present, then flags in a fixed order. The expanded token sequence used for display and for execution comes from one builder — the single source of truth — but the prompt token is rendered differently per surface: for execution it is a single argv token passed verbatim (no shell, no quoting needed); for display (`alias get`/`list`) it is shell-quoted so the shown `start ...` command is copy-pasteable. The argv sequence and the display string are otherwise identical.
...
5. Top-level resolution. Before cobra command dispatch, identify the first positional argument with persistent flags stripped (persistent flags may appear in any position and are passed through). When that token is not a known subcommand or subcommand alias and matches an alias name (case-insensitive), replace the token in place with the alias's expansion tokens and let the rewritten invocation run through cobra normally. All other arguments retain their position, so trailing arguments and flags are preserved:
   ...
   A trailing positional after a prompt alias needs no special handling. The rewrite preserves it in place, producing two positionals for `prompt`, and the multi-argument `prompt` command (see `01-prompt-arg-feature.md`) joins them with a single space into one body: `start foo "extra text"` runs a prompt of `this is the prompt extra text`. The alias resolver stays a pure argv rewrite that never inspects whether it produced a task or a prompt. Piped stdin follows the same precedence as a direct `prompt` invocation: because a prompt alias always supplies the fixed text as a positional, `echo x | start foo` ignores the piped `x` (argument wins over stdin).
...
7. Management command. A top-level command `alias` (with cobra alias `aliases`), global config only:
   ...
   - List and get render each alias as its name plus its full expanded `start ...` command — the same expansion used at resolution — not the raw stored value. A `prompt:` target is shell-quoted in this display so the shown command is copy-pasteable; `--json` carries the raw, unquoted prompt text instead.
   - Set takes two positional arguments, the name and the value; the value's composed targets are split on category anchors (`agents:`, `roles:`, `contexts:`, `tasks:`, `prompt:`), with commas/whitespace between targets tolerated as cosmetic separators (Requirement 2). A `prompt:` target consumes the remainder of the value verbatim and so must come last. The value is validated before any write; an invalid value writes nothing. Set is an upsert and preserves all other stored aliases.
...
8. Embedded schema and validation. Embed a `#Aliases` CUE schema in the binary for address shape, paired with a Go validator for the per-category cardinality limits and the `tasks:`/`prompt:` mutual exclusion CUE cannot express cleanly. The schema constrains the four module-category targets to the kebab-case path shape; the `prompt:` target is exempt from that regex, accepting any free-text string. Alias-entry validity means schema plus Go validator together. Validate through this one combined validator at three points: on write (reject a bad `set`), on the resolve path (validate the matched entry per the match-scoped rule in Requirement 6), and in `start doctor` (validate the file and report alias count and any problems). Sharing one validator across write, resolve, and doctor guarantees a hand-edited breach — an unknown category, an unqualified address, a cardinality overflow, or both a task and a prompt — is caught on the dispatch path the same way doctor catches it, so a breaching entry is never silently expanded.
...
2. Value parsing, validation, and dispatch. Tokenise the value by splitting on the known category anchors rather than on commas (Requirement 2): reuse `parseAddress` for the `category:path` split and category validation of the four module categories, add the `prompt` category as a terminal target whose value is the verbatim remainder, and apply a qualification check that rejects any colon-less address. Validate cardinality and the `tasks:`/`prompt:` mutual exclusion (Requirement 3), and build the deterministic expansion token sequence (Requirement 4). Provide a helper that renders the expansion as a displayable `start ...` command string, shell-quoting the `prompt` token while leaving the executed argv token verbatim. This builder is the single source of truth for both execution and display.
...
- The `prompt` target is the only free-text value, and the only place quoting matters. On execution it becomes one argv token injected verbatim — no shell re-parse, so commas, colons, spaces, and embedded quotes survive untouched. On display it must be shell-quoted so the printed `start ...` is copy-pasteable, and in `--json` it is emitted raw. Keep prompt text case-preserved through write/read; only names and module addresses are lowercased.
...
- A `prompt:` value containing commas, colons, and embedded quotes is stored verbatim, round-trips through `_aliases.cue`, and reaches the `prompt` argument unchanged on resolution; its case is preserved while alias names and module addresses are lowercased.
- Setting a value with both `tasks:` and `prompt:`, or with two `prompt:` targets, is rejected and writes nothing.
- `start alias get foo` and `start alias list` show the prompt shell-quoted in the expanded command, while `--json` carries the raw prompt text.
