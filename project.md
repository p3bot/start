...
Keying liveness on the installed-match state — not on "every `ensureIndex` call" — forces live exactly when an auto-install may occur (any surface, no installed match) while keeping `get`/`describe` and fallback-matched queries that are already satisfied on disk cache-gated rather than hitting the registry live every time.
...
2. Make `ensureIndex` resolve live only when the query has no installed match. Pass the resolver's already-computed installed-match state to `ensureIndex` (or split the decision so the caller signals it): no installed match forces a live resolve (effective bare-major path to `FetchIndex`); an installed match keeps the existing cache-gated path. Keep the graceful fallback, the conditional progress/debug line, and the best-effort cache write on a live resolve.
