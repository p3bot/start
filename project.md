...
9. Config integration.
   ...
   - `config open aliases` (and `alias`) opens `_aliases.cue` in `$EDITOR`.
   ...
   - `config export` includes aliases in exported configuration. Export walks the directory and streams each file's raw bytes (its directory listing includes underscore-prefixed files, so `_aliases.cue` is emitted), and because it never parses them, a malformed `_aliases.cue` exports verbatim without error.
