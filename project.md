...
| ---- | ------- | ---------- |
| 75 | Transient | Registry network I/O a retry could clear: index resolve/fetch (`internal/registry/index.go`), module fetch and source resolution where the failure is a 5xx/429 `HTTPError`, `ociregistry.ErrTooManyRequests`, or a `net.Error` (`internal/registry/client.go`); the registry-unavailable cause in `task.go`. Excludes registry client construction and "source location does not provide OS path" (internal → 1), a bad module path/version (→ 2/3), and a not-found condition surfaced inside `Fetch` (`modregistry.ErrNotFound` / 404 → 3) even though it shares the fetch code path |
...
7. Update tests for the renamed flag and the exit-code mapping (assert representative paths: registry-unreachable, bad module name, malformed user CUE, missing required arg, missing module), plus the `help schemas` drift guard. The guard runs all nine commands under `--json` and asserts structural shape — keys and types, not data values — against the documented topic. Run every command through `setupStartTestConfig` with `skipRegistry: true` so none reaches the live registry; the registry-backed commands (`library`, `search`, `update`, `doctor validate`, and whichever `doctor` checks probe the registry) additionally need a local cached-index fixture (via `internal/cache`) to emit a shape offline, while the `config`/`list` family is local regardless. JSON-mode capture is new harness work — `snapshots_test.go` renders only text-mode `describe`/`config get` today — so extend the harness rather than assume it covers this. Update the `AGENTS.md` command table and cross-reference `help schemas` from `help agents`.
